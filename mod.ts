...
export class GraphQLClient {
  private url: string;
  private options: RequestInit & { timeout?: number };

  constructor(url: string, options?: RequestInit & { timeout?: number }) {
    this.url = url;
  ...
  rawRequest<T = any, V = Variables>(
    ...
    const { headers, method = "POST", ...fetchOptions } = this.options;
    const { url } = this;
